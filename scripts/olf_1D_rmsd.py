...
def get_rmsd(psf_file, dcd_file, color):
    ...
    # RMSD calculation (using the first frame as the reference)
...
time_ns_1, rmsd_values_1, color_1 = get_rmsd(psf_file_1, dcd_file_1, '#822EFF')  # Light Blue
time_ns_2, rmsd_values_2, color_2 = get_rmsd(psf_file_2, dcd_file_2, '#F433FF')  # Dark Blue

# Plot RMSD for both systems
plt.figure(figsize=(18, 4))

# Add vertical lines to given frames
plt.axvline(x=4400 * 0.1, color='#BCC6CC', linestyle='--', linewidth=2, label='Frame 9431')  # Convert to ns
plt.axvline(x=12500 * 0.1, color='#BCC6CC', linestyle='--', linewidth=2, label='Frame 17903')  # Convert to ns
plt.axvline(x=9900 * 0.1, color='black', linestyle='--', linewidth=2, label='Frame 257')  # Convert to ns
plt.axvline(x=0 * 0.1, color='#52595D', linestyle='--', linewidth=2, label='Frame 10132')  # Convert to ns

plt.plot(time_ns_1, rmsd_values_1, label='Protein 1', color=color_1, linewidth=2)
plt.plot(time_ns_2, rmsd_values_2, label='Protein 2', color=color_2, linewidth=2)
